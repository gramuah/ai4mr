# Stellar mass and radius generalization experiment
    ...
    if target == 'M':
        ...
        level0.append(('nnet',
                        MLPRegressor(activation='relu', hidden_layer_sizes=(25, 25, 25, 25), learning_rate='adaptive',
                                    learning_rate_init=0.06, max_iter=1000, solver='sgd', alpha=0.0001,
        ...
        level0.append(('nnet',
                       MLPRegressor(activation='relu', hidden_layer_sizes=(25, 25, 25, 25), learning_rate='adaptive',
                                    learning_rate_init=0.09, max_iter=1000, solver='sgd', alpha=0.0001,
                                    random_state=0, verbose=False)))
...
def get_models(target):
    ...
    if target == 'M':
        ...
        models['nnet'] = MLPRegressor(activation='relu', hidden_layer_sizes=(25, 25, 25, 25), learning_rate='adaptive',
                                     learning_rate_init=0.06, max_iter=1000, solver='sgd', alpha=0.0001, random_state=0,
                                     verbose=True)
    elif target == 'R':
        # Neural Network (for the R)
        models['nnet'] = MLPRegressor(activation='relu', hidden_layer_sizes=(25, 25, 25, 25), learning_rate='adaptive',
                                  learning_rate_init=0.09, max_iter=1000, solver='sgd', alpha=0.0001, random_state=0,
                                  verbose=True)
...
###########################################################
# Mass and radius estimation -- Generalization experiment #
###########################################################
...
# Sort stars using their Mass or Radius
...
for name, model in models.items():
    ...
    # scores, y_pred = evaluate_model(model, X_train_norm, y_train, X_train_norm, y_train)
...
dump(models, 'experiments/generalization_models_' + target + '.joblib')
dump([X_test_norm, y_test, n_samples], 'experiments/generalization_test_data_' + target + '.joblib')
